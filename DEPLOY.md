# Deploy warthundercheats.net

Step-by-step guide to deploy the War Thunder Cheats static site to **warthundercheats.net** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **warthundercheats.net** DNS
...
3. Configure build settings:
   - **Project name:** `war-thunderscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=warthundercheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **warthundercheats.net** as the primary custom domain on the Pages project.

### Apex (warthundercheats.net)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.warthundercheats.net/*`
   - **Target:** `https://warthundercheats.net/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`warthundercheats.net`, `.net`, `.com`), and legacy path redirects.
...
- `https://warthundercheats.net/es/`
- `https://warthundercheats.net/war-thunder-cheats/`
- `https://warthundercheats.net/war-thunder-aimbot/`
- `https://warthundercheats.net/sitemap.xml`
- `https://warthundercheats.net/robots.txt`

Verify redirects:

- `http://warthundercheats.net` → `https://warthundercheats.net` (301)
- `https://www.warthundercheats.net` → `https://warthundercheats.net` (301)
- Legacy domains (e.g. `warthundercheats.net`) → `https://warthundercheats.net` (301)
- `/sitemap-index.xml` → `/sitemap.xml` (301)
- Legacy paths (e.g. `/fortnite-hacks/`) → War Thunder equivalents (301)

## 5. Google Search Console

1. Go to [Google Search Console](https://search.google.com/search-console).
2. **Add property** → choose **Domain** → enter `warthundercheats.net`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://warthundercheats.net/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `warthundercheats.net` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/war-thunder-cheats/`)
...
- [ ] Custom domain `warthundercheats.net` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `warthundercheats.net`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://warthundercheats.net`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/war-thunder-cheats/` requested for indexing
