...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-war-thunderhacks/assets';
...
const USER_SCREENSHOTS = [
	{
		...
		file: 'war-thunder-esp-player-tags.webp',
	},
	{
		source:
		...
		file: 'war-thunder-wallhack-skeleton.webp',
	},
	{
		source:
		...
		file: 'war-thunder-aimbot-sniper.webp',
	},
	{
		source:
		...
		file: 'war-thunder-aimbot-skeleton.webp',
	},
	{
		source:
		...
		file: 'war-thunder-esp-radar.webp',
	},
	{
		source:
		...
		file: 'war-thunder-cheats-combat.webp',
	},
];
...
const REMOVE_PREFIXES = [
	'war-thunder-battle-royale-combat',
	'war-thunder-capture point-fight',
	'war-thunder-cheats-aimbot',
	'war-thunder-cheats-cover',
	'war-thunder-cheats-esp-wallhack',
	'war-thunder-cheats-hero',
	'war-thunder-cheats-logo',
	'war-thunder-cheats-package',
	'war-thunder-header-art',
	'war-thunder-loadout-builder',
	'war-thunder-player-esp',
	'war-thunder-scav-run-combat',
	'war-thunder-scav-run-mode',
	'war-thunder-squad-fight',
	'war-thunder-verdansk-map',
];

async function removeOldWar ThunderImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'war-thunder-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `war-thunder-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'war-thunder-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'war-thunder-cheats-logo.png');
...
await removeOldWar ThunderImages();
